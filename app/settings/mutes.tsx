  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg, paddingTop: insets.top }]}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.title, { color: colors.text }]}>뮤트 목록</Text>
        <View style={{ width: 40 }} />
      </View>
        ...
        <FlatList
          ...
          renderItem={({ item }: { item: Profile }) => (
            <View style={[styles.row, { borderBottomColor: colors.border }]}>
              ...
              <TouchableOpacity onPress={() => unmute(item.id)} style={[styles.actionBtn, { borderColor: colors.border }]}>
                <Text style={[styles.actionText, { color: colors.text }]}>뮤트 해제</Text>
              </TouchableOpacity>
            </View>
          )}
          ListEmptyComponent={<View style={styles.empty}><Text style={[styles.emptyText, { color: colors.muted }]}>뮤트한 사용자가 없습니다.</Text></View>}
        />
      )}
    </View>
  );
